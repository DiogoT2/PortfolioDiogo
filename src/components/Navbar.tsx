    ...
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center w-full px-6 pt-6 pointer-events-none">
      <motion.nav
        ...
        className={`flex items-center justify-between transition-all duration-500 ease-out pointer-events-auto ${isScrolled
          ? "w-full max-w-3xl rounded-full px-6 md:px-8 py-3 shadow-lg dark:shadow-none bg-white/70 dark:bg-zinc-900/70 backdrop-blur-md border border-zinc-200 dark:border-zinc-800"
          : "w-full px-0 md:px-6 py-2 border-transparent bg-transparent dark:bg-transparent shadow-none"
          }`}
      >
        <div className="font-mono text-sm font-bold tracking-tighter group cursor-pointer text-zinc-900 dark:text-zinc-50">
          <span className="text-zinc-400 dark:text-zinc-500">{"<"}</span>
          <span className="group-hover:text-emerald-500 dark:group-hover:text-emerald-400 transition-colors">DIOGO</span>
          <span className="text-zinc-400 dark:text-zinc-500">{" />"}</span>
        </div>

        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-zinc-500 dark:text-zinc-400">
          <a href="#about" className="hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors">
            About
          </a>
          <a href="#projects" className="hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors">
            Projects
          </a>
          <a href="#contact" className="hover:text-zinc-900 dark:hover:text-zinc-50 transition-colors">
            Contact
          </a>
        </div>
